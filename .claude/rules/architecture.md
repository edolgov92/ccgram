...
- `output_pipeline/tui_dialogs.py` (ccgram-pro) — auto-answers Claude Code's OWN modals (resume-from-summary → full session; drafted bug report → dismiss, never send; feedback opt-out → Esc, keep). They block the input line, so an unanswered modal swallows the next forwarded message silently. Wired into `interactive_state`'s `handle_interactive_ui` guard.
